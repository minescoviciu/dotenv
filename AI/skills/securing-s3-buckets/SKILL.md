...
description: >-
  ...
  monitoring, and remediation of misconfigurations. Use when the user wants to secure
  a new bucket, audit an existing bucket, fix a security finding, configure encryption,
  or enable logging and monitoring. Do NOT use for general S3 data operations, S3
  Tables setup, or discovering existing data assets.
version: 1
---
...
- You MUST apply a bucket policy with a `DenyInsecureTransport` statement that denies `s3:*` when `aws:SecureTransport` is `false` — this is REQUIRED, not optional. Example:
  ...
  ```
- You MUST ask the user which logging option they want before step 4: S3 Server Access Logs (SAL) to CloudWatch (recommended), SAL to a general purpose bucket, or CloudTrail data events. Default to SAL to CloudWatch unless the user specifies otherwise.
- When the user chooses SAL to CloudWatch, You MUST set up per-bucket vended delivery (PutDeliverySource + PutDeliveryDestination + CreateDelivery) for the bucket being secured — this is required for that bucket's logs to reach CloudWatch.
- You MUST then check whether the account-level S3 Tables integration exists. If it does NOT exist, You MUST ask the user whether to create it (do not create it automatically). When asking, state that the S3 Tables copy is delivered at no additional storage charge beyond CloudWatch ingestion, lets them run SQL analytics in Athena and join access logs with S3 metadata for cost attribution, and mirrors the log group's retention (same data window in a SQL-queryable format). Note that creating it is a one-time per-account action that provisions an account-level `aws-cloudwatch` table bucket (requires `observabilityadmin:CreateS3TableIntegration`, `logs:AssociateSourceToS3TableIntegration`, `s3tables:CreateTableBucket`, `s3tables:PutTableBucketEncryption`, and `s3tables:PutTableBucketPolicy`) and that standard Athena/query rates apply.
...
- For the logging control, treat any one of SAL to CloudWatch, SAL to a general purpose bucket, or CloudTrail data events as sufficient; mark it NOT CONFIGURED only when none are active
